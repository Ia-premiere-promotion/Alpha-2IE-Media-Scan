            ...
            for span in all_spans:
                ...
                if any(word in aria_label.lower() for word in ['réaction', 'j\'aime', 'like', 'reaction']):
                    ...
                    if numbers:
                        ...
                        try:
                            num = int(num_str)
                            # Filtrer les années (> 3000) qui ne sont pas des likes
                            if num < 3000:
                                likes = max(likes, num)
                        except:
                            pass
                ...
                for selector in link_selectors:
                    ...
                    if link:
                        ...
                        if url:
                            ...
                            url_lower = url.lower()
                            is_valid = any(keyword.lower() in url_lower for keyword in self.page_keywords)
                            if not is_valid:
                                # Ce n'est pas un post de la page cible
                                return None
                            break
            except Exception as e:
